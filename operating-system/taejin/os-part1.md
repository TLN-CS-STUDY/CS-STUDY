...
## Scheduler
스케줄러는 프로세스를 어떤 큐에 넣고, 언제 CPU를 할당할지 결정하는 운영체제의 모듈이다.

### 장기 스케줄러 (Long-term Scheduler / Job Scheduler)

역할: 어떤 프로세스를 Ready Queue에 넣을지 결정한다 (즉, 디스크에서 프로그램이 메모리로 올라오는 것을 제어)

목적: 메모리에 올라간 프로세스의 수(Degree of Multiprogramming)를 조절한다.

> 현대 OS(Windows, Linux 등)에서는 장기 스케줄러를 거의 사용하지 않는다.
과거에는 메모리가 귀해서 장기 스케줄러가 입구 컷을 했지만, 지금은 프로그램 실행 시 바로 메모리에 올립니다.
대신 메모리가 부족해지면 중기 스케줄러가 프로세스를 Swap-out 시켜 조절하며, 단기 스케줄러가 0.몇 밀리초 단위로 CPU를 분배하는 구조가 일반적이다.

### 중기 스케줄러 (Medium-term Scheduler / Swapper)

역할: 메모리에 너무 많은 프로세스가 올라와 성능이 저하될 때, 일부 프로세스를 통째로 디스크로 Swap-out시킨다.

목적: 메모리 공간을 확보하여 시스템의 과부하를 방지

### 단기 스케줄러 (Short-term Scheduler / CPU Scheduler)

역할: 준비 큐에 있는 프로세스 중 어떤 프로세스에 CPU를 Dispatch할지 결정한다.

특징: 매우 빈번하게 호출되므로 속도가 매우 빨라야 함
...
작동 방식: 새로운 프로세스가 도착했을 때, 현재 실행 중인 프로세스의 남은 시간보다 새 프로세스의 실행 시간이 짧으면 CPU를 뺏어 할당한다.
...
## MLFQ
프로세스의 우선순위를 동적으로 조정한다. <br>
I/O를 자주 발생시키면 우선순위를 유지하고, CPU를 오래 사용하면 우선순위를 낮춘다.

- 장점: Response Time과 Turnaround Time의 최적화 (작업 유형에 따라 우선순위 조정)
...
## Lottery Scheduler
Proportional Share 방식의 확률적 스케줄링 방식이다. <br>
각 프로세스에 티켓을 나눠주고, 추첨을 통해 당첨된 프로세스에 CPU를 할당한다.

- 장점: 구현이 단순하고, 티켓 수를 조절함으로써 프로세스 간의 자원 점유율을 유연하게 조정할 수 있다. 결국은 모든 프로세스가 CPU를 할당 받게 된다.
- 단점: 티켓을 얼만큼 배분할지에 대한 공정성 문제 존재

## Stride Scheduling
확률이 아닌 Deterministic 계산법을 사용한 방식으로 각 프로세스는 실행될 때마다 자신의 pass 값을 stride만큼 증가시키며, 항상 pass 값이 가장 작은 프로세스를 선택한다. <br>
(Stride: $1 / \text{티켓 수}$에 비례하는 값)

- 장점: 확률적 요소가 배제되어 있기 때문에, 특정 시점에 어떤 프로세스가 실행될지 스케줄러의 동작을 예측 가능
- 단점: 프로세스 Pass값 관리에 따른 overhead 증가, 티켓 배분에 대한 공정성 문제는 여전히 해결x

## The Linux Completely Fair Scheduler (CFS)
프로세스가 CPU를 얼마나 사용했는지를 기록하는 **vruntime**이라는 지표를 사용하여 스케줄러는 항상 vruntime이 가장 적은 프로세스를 선택하여 실행하는 스케줄링 방식

- 다음 프로세스 탐색을 위해 Red-Black Tree로 프로세스 관리
- Target Latency(스케줄링 주기)를 현재 실행 중인 프로세스 수로 나누어 Time Slice결정
- min_granularity을 두어 프로세스 최소 실행 시간을 보장하여 context switch overhead가 너무 커지는 것을 방지

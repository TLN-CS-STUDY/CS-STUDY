...
메모리 계층이란 필요에 따라 여러가지 종류로 나눈 것으로 레지스터, 캐시, 주기억장치(메모리), 보조기억장치(디스크)로 구성되어 있다.
...
- 캐시는 일반적으로 여러 계층으로 구성되며, 용량과 엑세스 속도에 따라 나뉜다.
	- L1 캐시: CPU 코어에 가장 가까운 곳에 위치하며, 용량은 작지만 빠른 엑세스 속도를 가진다.
	- L2 캐시: L2 캐시는 L1 캐시의 다음 계층에 위치하며, 용량은 더 크지만 엑세스 속도는 상대적으로 느리다.
	- L3 캐시: L3 캐시는 주로 멀티코어 프로세서에서 사용되며, 용량은 크지만 엑세스 속도는 L2에 비해 더 느리다.
	
### 3.1.3. 주기억장치
- 메인 메모리라고도 불리며, 컴퓨터 시스템에서 가장 중요한 메모리 계층 중 하나이다.
- 프로세서가 실제로 엑세스하는 데이터와 명령어를 저장하는 곳이다.
- 대표적으로 RAM과 ROM이 있다.

  - RAM(Random Access Memory): 휘발성 기억 장치
    - 빠른 액세스를 하기 위해 데이터를 단기간 저장하는 구성 요소이다.
    - 어느 위에서든 똑같은 속도로 접근하여 읽고 쓸 수 있다.

  - ROM(Read Only Memory): 고정 기억 장치
    - 컴퓨터에 지시사항을 영구적으로 저장하는 비휘발성 메모리이다.

### 3.1.4. 보조기억장치
- 주기억장치보다 용량이 크고 데이터를 영구적으로 저장하는 역할을 한다.

- HDD(Hard Disk Drive)
  - 비휘발성, 순차접근이 가능한 컴퓨터의 보조기억장치이다.
